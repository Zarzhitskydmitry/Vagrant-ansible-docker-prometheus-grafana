...
Vagrant.configure(2) do |config|			        # начинает цикл config
  ...
  config.vm.provider "virtualbox" do |vb|			# используем провайдер VBox без gui и что памяти мы готовы выделить не больше 1 Гб
     vb.gui = false
     vb.memory = "1024"
  config.vm.hostname = "testing"                       # Указываем имя пользователя для подключения по ssh
  config.vm.network "private_network", ip: "192.168.0.196" 	# задаем приватный адрес, с которым он будет работать 
  config.vm.network "forwarded_port", guest: 3000, host: 3000, auto_correct: true    # открывает наши порты на виртульной машины к хосту
  ...
  config.ssh.forward_agent = true        # открываем подключение к агенту ssh
  config.ssh.username = 'vagrant'        # назначаем пользователя для подключения ssh
  config.ssh.password = 'vagrant'        # назначаем пароль пользователю для подключения ssh
  config.ssh.keys_only = false            # используование только закрытые ключи SSH
  config.vm.provision "ansible" do |ansible|			# Задаем какой плейбук отработать при запуске
    ansible.limit = "all"            # отключает лимит подключения
    ansible.inventory_path = "hosts.txt"        # используем наш инвентори файл
    ansible.playbook = "ansible_vagrant_playbook.yml"        # название нашего плейбука
  end
  end
end
...
192.168.0.196                           # Ip адрес или ссылка на него например host.example.com

[pc:vars]                                 # Название группы и переменные для доступа. Тут записываются все переменные как достучаться до хоста
ansible_user=vagrant                       # Имя пользователя
ansible_become_password=vagrant             # Пароль подтверждения
ansible_port=22
ansible_ssh_private_key_file='/home/%YOURUSERNAME%/testovoe/.vagrant/machines/testing/virtualbox/private_key'    # И ключ по которому мы подключимся к узлу по ssh
 ```
...
- (IP выашего хоста):9100 - node_exporter
- (IP выашего хоста):9000 - prometheus
- (IP выашего хоста):3000 - grafana
